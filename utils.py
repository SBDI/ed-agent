...
def restart_agent():
    ...
    st.session_state["ed_agent_session_id"] = None
    st.session_state["messages"] = []
    st.rerun()
...
def export_chat_history():
    ...
    if "messages" in st.session_state:
        chat_text = "# Ed Agent - Chat History\n\n"
...
def sidebar_widget() -> None:
    """Display a sidebar with sample user queries for Ed Agent."""
    with st.sidebar:
        st.markdown("#### 📜 Try me!")
        ...
        with col1:
            if st.button("🔄 New Chat"):
                restart_agent()
        with col2:
            fn = "ed_agent_chat_history.md"
            if "ed_agent_session_id" in st.session_state:
                fn = f"ed_agent_{st.session_state.ed_agent_session_id}.md"
...
def session_selector_widget(agent: Agent, model_id: str) -> None:
    ...
    if agent.storage:
        ...
        if st.session_state.get("ed_agent_session_id") != selected_session_id:
            logger.info(
                f"---*--- Loading {model_id} run: {selected_session_id} ---*---"
            )
            # Get model name from environment or use the provided model_id
            model_name = os.environ.get("MODEL_NAME", "llama-3.3-70b-versatile")
            env_model_id = f"groq:{model_name}"

            st.session_state["ed_agent"] = tutor_agent(
...
def about_widget() -> None:
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("### ℹ️ About Ed Agent")
    st.sidebar.markdown(
        """
        Ed Agent is an educational AI assistant that delivers personalized learning experiences tailored to your education level.
